...
func JsonSerialize() {
	var user = User{Name: "dada", Age: 18, height: 175, Birthday: time.Now(), CreatedAt: MyDate(time.Now()), Sex: 1, Address: struct {
		Province string
		City     string
	}{Province: "浙江", City: "杭州"}}
	...
	if err != nil {
		...
		fmt.Printf("反序列化成功 %+v\n", u)
		//使用Println直接打印user时会调用它的String()方法，user的String()又会调用各个成员变量的String()
	}
}
